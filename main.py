...
for i in range(0,4):
    for j in range(0,10):
        # 账号信息
        param = {
            "stuNum": "21080105"+str(i)+str(j),  # 此处输入学号
            "pwd": "Lc2#0000000000",  # 此处输入密码
            "vcode": "",
        }
        #
        header = {
        ...
        if y == '已完成':
            ...
            try:
                ...
                a = "填报成功"
                print("填报成功"+str(i)+str(j))
            except:
                a = "填报出错"
                print("填报出错" + str(i) + str(j))


        else:
            z = "填写时间未到，或填写失败"
            print("填写时间未到，或填写失败" + str(i) + str(j))
